...
- re-parenting with `uebergeordnete_aufgabe` repairs the first case as it happens, so such a
...
`update_task` each (`uebergeordnete_aufgabe` to re-point, `felder_leeren:
["uebergeordnete_aufgabe"]` to make them top-level).
