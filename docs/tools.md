...
(`"method"` is `"MOVE"` if CalDAV MOVE was used, or `"copied"` if copy+delete fallback was executed. `"hierarchy"` is only present when the task link was changed: `"set"` for a new `linked_task`, `"cleared"` for `clear_fields`.)
