...
class _TaskLink(NamedTuple):
    ...
    top-level task); `title` is its SUMMARY, carried along so that reporting a
...
_EVENT_BATCH = _BatchKind("VEVENT", "calendar_name", "event_uids", "Event", InvalidEventDataError)
...
def _aborted_batch(
    ...
    But an exception carries no `results`, and "which of the thirteen went
...
class CalDavService:
    ...
    def move_task(
        ...
        Returns:
            {"uid": task_uid, "from": source, "to": target,
            "method": "MOVE" | "copied" | "already_there"}, plus
            "hierarchy": "set" | "cleared" when the parent was changed, plus
            "orphaned_subtask_links" (see `_orphaned_subtask_links`).
        """
        ...
        with self._lock:
            ...
            if parent_task is not None or clear_parent:
            ...
            result["orphaned_subtask_links"] = self._orphaned_subtask_links(
    ...
    def _orphaned_subtask_links(
        ...
        `move_task`'s own `parent_task`/`clear` runs before this
        ...
        `{"uid", "title", "list", "fehlende_parent_uid"}`. An empty
            ...
            orphans.append(
                {
                    ...
                    "title": moved.title,
                    "list": target_display,
                    "fehlende_parent_uid": moved.parent,
                }
            )
    ...
    def _validate_task_patch(fields: mapping.TaskFields) -> None:
        ...
        every check `apply_task_fields` performs - unknown `clear_fields`
        ...
        The probe carries a DTSTART because relative reminders and `recurrence`
        both validate against having one, and its value kind follows the patch's
        own `due_date` so an all-day due date isn't judged against a timed
        ...
        if fields.start_date is None:
            # With `start_date` in the patch, `apply_task_fields` sets DTSTART
            # on the probe itself before it validates anything against it.
            all_day_due = (
                fields.due_date is not None and _ALL_DAY_RE.match(fields.due_date) is not None
            )
    ...
    def move_tasks(self, list_name: str, task_uids: list[str], target_list: str) -> dict[str, Any]:
        ...
        "already_there" instead of being moved again or reported as missing.
        """
        with self._lock:
    ...
    def _batch_over_uids(
        ...
        clashes in the target) becomes an entry in `results` and the loop
                    ...
                    results.append(
                        {
                            "uid": uid,
                            "status": "error",
                            "error": f"{kind.noun} '{uid}' was not found.",
                        }
                    )
                    failed += 1
                except Exception as exc:
    ...
    def move_event(
        ...
        Returns:
            {"uid": event_uid, "from": source, "to": target,
            "method": "MOVE" | "copied" | "already_there"}, plus
            "hierarchy": "set" | "cleared" when the link was changed.
        """
    ...
    def _move_object(
        ...
        "already_there" recovery, which together make a lost acknowledgement
        ...
        with self._lock:
            ...
            try:
                ...
                if status in _TRANSIENT_HTTP_STATUSES:
                    ...
                    # the object in the target and reports "already_there".
